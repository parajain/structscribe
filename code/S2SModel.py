...
class ApproxNN(S2SModel):
    def __init__(self, filename='/data1/struct2text/data/v3_further_processed/Facts_Tax_OpenIE_verbnet_data_filterrels_validents_subset_PROCESSED.tsv'):
        bAngular, bHamming, bEuclidean, bManhattan, \
        col1list, col1map, col2list, col2map, sentmap, glovemap = \
            initializeAnnoy('/data1/struct2text/Anirban/glove.840B.300d.processed.txt', filename)
            #initializeAnnoy('/data1/data1/Abhijit/Coherence/Data/glove.6B.300d.txt', filename)
...
class Seq2Seq(S2SModel):
    #def __init__(self, modelfile='/data1/struct2text/s2s_models_v3/model_softmax_acc_96.95_ppl_1.58_e3.pt'):
    def __init__(self, modelfile='/data1/struct2text/s2s_models_v3/model_softmax_acc_97.30_ppl_1.41_e7.pt',
                 dynamic_dict=True, attn_debug=True, share_vocab=True, replace_unk=True, verbose=True):
    ...
    def generate(self, input):
        #Assuming multiple input string are concatenated with delimiter '####'
        ftemp = codecs.open(self.opt.src, 'w', "utf-8")
        inputs = input.strip().split('####')
        for inp in inputs:
            ftemp.write(inp.strip() + '\n')
        ftemp.close()
        ...
        data_iter = onmt.io.OrderedIterator(
            ...
            batch_size=128, train=False, sort=False,
        ...
        for batch in data_iter:
            ...
            for trans in translations:
                ...
                out_file.flush()
                output_sentence = '\n'.join(n_best_preds)
                out_sents.append(output_sentence)
